...
The orchestrator handles all iterations automatically. It runs the policy rollout and retraining unattended, and pauses once per iteration to prompt you to play the level:
...
To resume after an interruption (e.g. crashed on iteration 4):
...
artifacts/
    ...
    dagger_labeled/iterN.npz      ← human-labeled states
    dagger_dataset_iterN/         ← aggregated training shards
    dagger_iterN.pt               ← retrained checkpoint
```

Typically run 5–10 iterations. Stop when the policy consistently clears the level or performance stops improving.

---

### What the loop does under the hood

Each iteration follows the same three steps, advancing the checkpoint chain:

```
bc_warmup.pt  →  dagger_iter1.pt  →  dagger_iter2.pt  →  ...  →  dagger_iter8.pt
```

**Step 1 — Policy rollout (automated):** the current policy plays the game for 50 episodes unattended, recording every state it visits including failure states. Uses the previous iteration's checkpoint as the policy.

**Step 2 — You play the level:** you play for 5–10 minutes while `record_human.py` runs, providing human labels at every X position the policy reached in Step 1.

**Step 3 — Align, aggregate, retrain (automated):** visited states are matched to human labels by X position (`dagger_align.py`), merged with all past data into a growing dataset (`dagger_dataset.py`), and the policy is retrained from scratch on the combined data (`bc_train.py`). Old data is never discarded.
